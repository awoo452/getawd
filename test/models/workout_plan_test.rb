...
class WorkoutPlanTest < ActiveSupport::TestCase
  ...
  test "creates a task on save" do
    assert_difference "Task.count", 1 do
      WorkoutPlan.create!(planned_on: Time.zone.today, workout_type: :walk)
    end
  end
  ...
  test "vr generates correct task name" do
    wp = WorkoutPlan.create!(planned_on: Time.zone.today, workout_type: :vr)
  ...
  test "board_pull generates correct task name" do
    wp = WorkoutPlan.create!(planned_on: Time.zone.today, workout_type: :board_pull)
    assert_equal "Strength — Shoulders & Back", wp.task.task_name
  end

  test "rest generates correct task name" do
    wp = WorkoutPlan.create!(planned_on: Time.zone.today, workout_type: :rest)
