module Kitchen
  class IndexData
    ...
    def call
      ...
      active_recipes = Recipe.active.includes(recipe_ingredients: :food_item).ordered.to_a
      recipes_by_slot = active_recipes.group_by(&:meal_type_suggestion)
      ...
      Result.new(
        ...
        recipes_by_type:         active_recipes.group_by(&:meal_type_suggestion),
        ...
        recipes_for_slot:        MealPlan::SLOTS.index_with { |slot| recipes_by_slot[slot] || [] },
        food_items_grouped:      FoodItem.active.ordered.group_by { |fi| fi.food_type.humanize }.transform_values { |items| items.map { |fi| [fi.name, fi.id] } },
